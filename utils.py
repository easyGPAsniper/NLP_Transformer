...
def beam_search_decode(model, src, src_mask, max_len, start_symbol, beam_size, end_idx):
    ...
    for i in range(max_len - 1):
        ...
        candidates = scores.unsqueeze(1) + prob
        
        # Reshape candidates tensor and get top k scores
        flat_candidates = candidates.view(-1)
        topk_scores, topk_indices = torch.topk(flat_candidates, k=beam_size, dim=-1)
        
        # Extract top k sequences
        ys_indices = topk_indices // vocab_size
        token_indices = topk_indices % vocab_size
        
        # Update ys tensor
        ys = torch.cat([ys[ys_indices], token_indices.unsqueeze(1)], dim=-1)
        
        # Update scores tensor
        scores = topk_scores
        ### YOUR CODE ENDS HERE #######
