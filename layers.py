...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query = self.linears[0](query).view(nbatches, l_q, self.h, self.d_k).transpose(1, 2)
        key = self.linears[1](key).view(nbatches, l_k, self.h, self.d_k).transpose(1, 2)
        value = self.linears[2](value).view(nbatches, l_k, self.h, self.d_k).transpose(1, 2)

        x, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout)

        x = x.transpose(1, 2).contiguous().view(nbatches, l_q, self.h * self.d_k)
        x = self.linears[3](x)
